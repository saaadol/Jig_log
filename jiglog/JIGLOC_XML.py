...
def convert_to_xml(file, output_directory):
    ...
    workbook1 = openpyxl.load_workbook(f'{browse_matching_file_function()}')
...
def process_files(input_path, output_path):
    ...
    print("Conversion completed. XML files are saved in:", output_path)
    window.destroy()
...
background_image_path = f"{current_directory}\\safrannossocietesdefjpg.jpg"  # Replace with the actual path to your background image
...
browse_button = tk.Button(window, text='Browse Files', command=browse_files, width = 15)
browse_button.place(relx=0.85, rely=0.22, anchor=tk.CENTER)

browse_matching_file = tk.Button(window, text='Browse matching', command=browse_matching_file_function, width = 15)
browse_matching_file.place(relx=0.85, rely=0.62, anchor=tk.CENTER)
